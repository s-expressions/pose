...
* It's possible to construct a list where the second part of the last
  ...
  _improper lists_ or _dotted lists_, and the last (possibly the only)
  pair is a _dotted pair_.
...
case; R^6^RS (2007) was the Scheme standard that specified lowercase
by default. Almost all Scheme implementations are now case sensitive
by default, and use lowercase.
...
comment     = ';' and all subsequent characters up to a line ending

expression  = string | symbol | number | list
symbol      = barsym | word
list        = '(' expressions ')'
barsym      = '|' barsym-char* '|'
string      = '"' string-char* '"'
barsym-char = barsym-esc | literal character
string-char = string-esc | literal character
barsym-esc  = \\ | \|
string-esc  = \\ | \"
word        = word-first word-cont*
word-first  = a-z A-Z + - * / % & < = > _
word-cont   = word-first | 0-9 . ! ? $

number      = '0' | decimal | bin | oct | hex
bin         = '#' 'b' sign? bin-digits
oct         = '#' 'o' sign? oct-digits
hex         = '#' 'x' sign? hex-digits
decimal     = sign? 1-9 0-9* fraction? exponent?
fraction    = '.' dec-digits
exponent    = 'e' sign? dec-digits
sign        = '+' | '-'
----
