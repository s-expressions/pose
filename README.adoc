...
### Case sensitivity in symbols (DISPUTED)
...
implementations are following suit. Hence, all symbols
are case sensitive in POSE. Readers that need to interoperate
with the legacy uppercase symbols may relax this requirement, but that
is non-normative.

#### Argument con

Since most data is sent using lower case symbols only, a case-insensitive
system will receive `(foo bar)` as `(|foo| |bar|`, which is very irritating
to program with: code becomes littered with symbols in vertical bars,
the vertical-bar key gets stuck when writing such code, and the programmer
curses the spec authors.  Allowing symbols to be sent in any mixture of
cases, but allowing the receiver to normalize them to a preferred case
(upper case for CL) is easier.
...
to avoid dropping them into a random package when they are received.
However, a single `:` is not a POSE symbol, nor is a token beginning
with more than one `:`.  Pose makes no semantic distinctions between
symbols that begin with `:` and those that do not.
